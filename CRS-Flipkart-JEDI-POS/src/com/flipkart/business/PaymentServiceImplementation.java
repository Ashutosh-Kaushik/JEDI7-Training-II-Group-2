...
public class PaymentServiceImplementation implements PaymentServiceInterface{
    ...
    public void payFees(Student student) throws SQLException {
        String sql = "SELECT `registrar`.`userId`,\n" +
                "    `registrar`.`courseId`,\n" +
                "    `registrar`.`grade`\n" +
                "FROM `crs`.`registrar`;\n";
        ...
        System.out.println("1. " + PaymentConstants.UPI);
        System.out.println("2. " + PaymentConstants.CARD_PAYMENT);
        ...
        if(pType == 1) {
            ...
            upi = s.next();
            pdi.insertIntoBookkeeper(paymentId,PaymentConstants.UPI_STRING,student.getSemester());
        }
        else if(pType == 2) {
            String paymentId = this.getRandomString();
            ...
            System.out.println("Enter Pin");
            upi = s.next();
            pdi.insertIntoBookkeeper(paymentId,PaymentConstants.CARD_PAYMENT_STRING,student.getSemester());
        }
        else {
